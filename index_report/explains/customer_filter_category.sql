EXPLAIN ANALYZE select id,name from users order by name asc offset 0 limit 20;
EXPLAIN ANALYZE select id,name from products where cid=1 order by name asc offset 0 limit 10;
...
EXPLAIN ANALYZE VERBOSE select s.uid, sum(s.quantity*s.price) from  u_t u, sales s  where s.uid=u.id group by s.uid;
EXPLAIN ANALYZE VERBOSE select s.pid, sum(s.quantity*s.price) from p_t p, sales s where s.pid=p.id  group by s.pid;
