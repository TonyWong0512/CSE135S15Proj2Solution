...
EXPLAIN ANALYZE VERBOSE select st.name, st.id, coalesce(sum(s.quantity*s.price),0) as price from s_t st LEFT OUTER JOIN ( select u.state as state, s2.price as price, s2.quantity as quantity  from users u, sales s2, products p where u.id = s2.uid and s2.pid = p.id and p.cid = 1) s on s.state = st.id group by st.id, st.name;
