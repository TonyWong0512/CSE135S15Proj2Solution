...
EXPLAIN ANALYZE VERBOSE insert into p_t (id, name, price) select s.pid, p.name, sum(s.quantity*s.price) as price from products p LEFT OUTER JOIN sales s ON s.pid=p.id WHERE p.cid = 1 group by s.pid, p.name order by price offset 0 limit 10;
