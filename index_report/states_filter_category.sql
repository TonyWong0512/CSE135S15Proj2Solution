...
EXPLAIN ANALYZE VERBOSE select st.name, st.id, sum(s.quantity*s.price) as price from s_t st LEFT OUTER JOIN ( select u.state as state, s2.pid as pid, s2.price as price, s2.quantity as quantity  from users u, sales s2, products p where u.id = s2.uid and s2.pid = p.id and p.cid = 1) s on s.state = st.id group by st.id, st.name;
EXPLAIN ANALYZE VERBOSE select p.name, s.pid, sum(s.quantity*s.price) as price from products p, sales s where s.pid=p.id  group by s.pid, p.name order by price desc offset 0 limit 10;
