...
EXPLAIN ANALYZE VERBOSE select s.state, sum(s.quantity*s.price) from s_t LEFT OUTER JOIN (select u.state as state, s.quantity as quantity, s.price as price FROM users u JOIN sales s ON s.uid=u.id) s ON s.state=s_t.id group by s.state;
