...
public class ProductsHelper {
	...
	public static List<ProductWithCategoryName> listProducts(
		...
		try {
			...
			String query = "WITH selected AS (SELECT * FROM products"
					+ filter
			...
			while (rs.next()) {
				...
				productWithCategoryNames.add(new ProductWithCategoryName(id,
						cname, name, SKU, price));
			}
	...
	public static String modifyProducts(HttpServletRequest request) {
		...
		try {
			...
			try {
				...
				if (cname != null) {
					...
					if (rs.next()) {
						...
						return HelperUtils
								.printError("Category chosen does not exist!");
					}
				}
			} catch (Exception e) {
				name = null;
			...
			if (("insert").equals(action)) {
				String SQL_1 = "INSERT INTO products (cid, name, SKU, price) VALUES("
						+ cid
						+ ",'"
						+ name
						+ "','"
				...
				String SQL_2 = "update products set name='" + name
						+ "' , SKU='" + sku + "'  , price='" + price
						+ "' where id=" + id_str + ";";
				try {
					conn.setAutoCommit(false);
